...
// 1. 이미지를 FormData로 형변환, 자바가 인식할수 있도록 요건 type을 지정할 필요가 없는 이윤 자바에서 다 하나하나 뭐시기하기떄문?
  ...
  return (
    <div className="flex flex-col content-center items-center h-full">
      ...
      <section>
          <textarea className="pl-4 pt-4 border reszie-none border-gray rounded-2xl w-[500px] h-60" value={reportDetail}
      ...
      <section>
		   ...
		   <input type="file" id="file" multiple accept=".jpg, .jpeg, .png" placeholder="제목을 작성해주세요" className="hidden" onChange={handleImageChange}/>
     { reportImage.length ? (
	  <>
	  {reportImage.map((file, i) => (
		<div key={i}
		 className="flex justify-center border border-gray rounded-2xl w-[500px] h-40 pt-5 pl-5">
      ...
      <section className="flex justify-center mb-[50px] mt-[100px] gap-10">
				...
				<button className="font-bold bg-mainColor text-white h-14 rounded-2xl w-[200px]" onClick={handleReportPost}>
           			 신고
				</button>
	  </section>
